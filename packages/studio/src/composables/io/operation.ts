...
export function mergeTinyI18nUpdatePatch(
  ...
  const previousTranslations = (
    previous as Partial<TinyI18nMessage> | undefined
  )?.translations
...
function findCreatedOperation(operations: TinyI18nOperation[], id: string) {
  return operations.find(
    operation => operation.type === 'create' && operation.data.id === id,
  )
}
 ...
 */
function removeOperationsByIds(
  operations: TinyI18nOperation[],
  ids: Set<string>,
) {
...
export function compactTinyI18nOperations(operations: TinyI18nOperation[]) {
  return operations.reduce<TinyI18nOperation[]>((nextOperations, operation) => {
    ...
    if (operation.type === 'update') {
      const createdOperation = findCreatedOperation(
        nextOperations,
        operation.data.id,
      )
      ...
      if (createdOperation?.type === 'create') {
        ...
        createdOperation.data = applyUpdateToItem(
      ...
      if (
        lastOperation?.type === 'update'
        && lastOperation.data.id === operation.data.id
      ) {
