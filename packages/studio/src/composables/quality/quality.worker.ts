...
function buildNodeMaps(items: TinyI18nItem[]) {
  ...
  function resolvePath(id: string): string {
    ...
    const path = item.parent
      ? `${resolvePath(item.parent)}.${item.key}`
      : item.key
...
const workerScope = globalThis as typeof globalThis & {
  onmessage: ((event: MessageEvent<QualityWorkerPayload>) => void) | null
  postMessage: (message: TinyI18nQualityReport) => void
}

workerScope.onmessage = (event: MessageEvent<QualityWorkerPayload>) => {
  workerScope.postMessage(scanQuality(event.data))
}
