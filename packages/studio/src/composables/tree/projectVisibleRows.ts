...
export function projectVisibleRows(
  ...
  function hasMatchedDescendant(nodeId: string) {
    ...
    return (tree.descendantsById.get(nodeId) ?? []).some((node) => matchedIds.has(node.id));
  }
  ...
  function visit(node: LocaleTreeGroupNode) {
    ...
    const shouldInclude = !matchedIds || matchedIds.has(node.id) || hasMatchedDescendant(node.id);
