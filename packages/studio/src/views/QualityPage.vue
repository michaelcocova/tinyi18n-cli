...
const { report, error, refresh } = useQualityCheck()

const summaryCards = computed(() => [
  {
    label: '已扫描消息',
    value: report.value.scannedMessages,
    icon: ScanSearch,
  },
  {
    label: '缺失 key',
    value: report.value.counts.missing_key,
    icon: FileWarning,
  },
  {
    label: '缺失翻译',
    value: report.value.counts.missing_translation,
    icon: Languages,
  },
  {
    label: '空翻译',
    value: report.value.counts.empty_translation,
    icon: AlertTriangle,
  },
  {
    label: '全局重复',
    value: report.value.counts.duplicate_path_global,
    icon: CopyMinus,
  },
  {
    label: '分组内重复',
    value: report.value.counts.duplicate_path_group,
    icon: CopyMinus,
  },
])
    ...
    <section class="mx-auto w-full max-w-6xl">
      ...
      <div class="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        >
          ...
          <strong class="mt-3 block text-2xl font-semibold text-zinc-950">{{
            card.value
          }}</strong>
        </article>
      </div>
