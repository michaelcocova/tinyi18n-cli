...
const generatedCode = computed(() => {
  ...
  const configString = JSON.stringify(config, null, 2).replace(/"([^"]+)":/g, "$1:");

  return `import { defineConfig } from "tinyi18n";\n\nexport default defineConfig(${configString});\n`;
});
...
<template>
  <div class="min-h-svh bg-background flex items-center box-border">
    <div class="mx-auto max-w-5xl w-full gap-6 grid lg:grid-cols-[1fr_400px] items-start">
      <div class="flex-1 flex flex-col min-h-svh justify-center py-10">
        <h1 class="text-xl font-bold tracking-tight sm:text-3xl">欢迎使用 TinyI18n</h1>
        <p class="mt-5 max-w-md text-base leading-relaxed text-muted-foreground">
          让我们通过几个简单的步骤来初始化你的工作区配置。
        </p>
        <SetupForm :isSubmitting="isSubmitting" @submit="onSubmit" />
      </div>
