  ...
  (entries) => {
    ...
    if (
      ...
      !entriesState.value.some(
        (entry) =>
          entry.key === selectedNodeId.value || entry.key.startsWith(`${selectedNodeId.value}.`),
      )
    ) {
...
const navigationRows = computed<VisibleRow[]>(() => visibleRows.value.filter((row) => !row.isLeaf));
...
function insertNode(nodes: TreeNode[], segments: string[], entry: LocaleEntryLike, depth: number) {
...
function flattenVisibleRows(nodes: TreeNode[], expanded: Record<string, boolean>): VisibleRow[] {
...
function isTerminalGroup(node: TreeNode): boolean {
  return node.children.length > 0 && node.children.every((child) => child.children.length === 0);
}
...
function updateValue(entry: LocaleEntryLike, language: string, nextValue: string | number) {
...
async function selectNode(nodeId: string | null) {
  ...
  const targetIndex = visibleRows.value.findIndex((row) => row.node.id === nodeId);
  if (targetIndex >= 0) {
  ...
  <div class="space-y-4">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <Button variant="outline" @click="expandAll">全部展开</Button>
        <Button variant="outline" @click="collapseAll">全部收起</Button>
        <Button :disabled="pending" @click="save">
          {{ pending ? "保存中..." : "保存全部修改" }}
        </Button>
      </div>
    </div>
    ...
    <div class="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
      <aside class="overflow-hidden rounded-xl border bg-card">
        <div class="border-b px-4 py-3">
          <p class="text-sm font-medium">目录导航</p>
          <p class="mt-1 text-xs text-muted-foreground">点击节点快速定位右侧对应位置</p>
        </div>
        <div class="max-h-[620px] overflow-auto p-2">
          ...
          <div class="mt-2 space-y-1">
            <div v-for="row in navigationRows" :key="`nav:${row.node.id}`">
              <div
                class="flex items-center gap-1 rounded-md px-2 py-1"
                :class="selectedNodeId === row.node.id ? 'bg-accent text-accent-foreground' : ''"
              >
                <button
      ...
      <div class="overflow-hidden rounded-xl border bg-card">
        ...
        >
          <div class="px-4 py-3">键名</div>
          <div v-for="language in languages" :key="language" class="border-l px-4 py-3">
            {{ language }}
          </div>
        </div>

        <div ref="contentContainer" class="max-h-[620px] overflow-auto [overflow-anchor:none]">
          <Virtualizer
              ...
              <template v-if="row.isLeaf">
                ...
                >
                  <Input
                    ...
                    "
                    @update:model-value="updateValue(row.node.entry!, language, $event)"
                  />
                </div>
              </template>
            </div>
