...
import { CheckCircle2, FileJson, AlertCircle } from "@lucide/vue";
...
<template>
  <div class="flex w-full flex-col gap-3 rounded-lg border bg-background p-4 shadow-lg w-[420px] pointer-events-auto">
    <div class="flex items-center gap-2 text-emerald-600">
      <CheckCircle2 class="size-5" />
      <h3 class="font-semibold text-sm">所有文件生成完成！</h3>
    </div>
    ...
    <div class="space-y-3 text-xs">
      ...
      <div class="rounded-md border bg-muted/30">
        <div class="flex items-center gap-1.5 font-medium text-foreground p-2.5 border-b bg-muted/50">
          <FileJson class="size-3.5" />
          文件详情
        </div>

        <div class="p-2 max-h-[240px] overflow-y-auto custom-scrollbar">
          <div v-for="(group, idx) in summary.groups" :key="idx" class="mb-3 last:mb-0">
            <div class="text-xs font-mono text-muted-foreground mb-1.5 px-1 truncate" :title="group.dir">
              {{ group.dir }}/
            </div>
      ...
      <div v-if="skipped.length > 0" class="flex items-start gap-2 pt-1 text-amber-600">
