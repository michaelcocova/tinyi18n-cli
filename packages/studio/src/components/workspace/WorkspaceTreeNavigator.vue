...
const { expandedIds, toggleExpanded, isExpanded } = useGroupNavigatorExpansion();
...
function scrollToNode(id: string) {
  window.dispatchEvent(new CustomEvent("workspace:scroll-to-message", { detail: { id } }));
  open.value = false;
}
</script>
...
<template>
  <Popover>
    <PopoverTrigger as-child>
      <Button variant="ghost" size="icon-sm" class="size-6 data-[state=open]:bg-accent">
        <LocateFixed />
      </Button>
    </PopoverTrigger>

    <PopoverContent class="text-sm min-w-50 p-1 overflow-y-auto" align="start" side="bottom">
      <div
      ...
      >
        ...
        <span class="text-slate-400" v-if="item.original.key">
