...
<template>
  ...
  >
    <Badge variant="secondary" class="px-1.5 text-muted-foreground font-normal select-none">
      已选择
      {{ checkedCount }}
      条
    </Badge>
    ...
    <WorkspaceGroupSelectMenu
      ...
      @select="handleMove"
    />
    <Button size="icon-sm" variant="ghost" @click="handleDelete">
      <Trash2 />
    </Button>
    <Button size="icon-sm" variant="ghost" @click="clearChecked">
      <X />
    </Button>
  </div>
</template>
