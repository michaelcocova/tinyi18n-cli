...
import { CheckSquare, ChevronRight, FolderTree, Languages, MinusSquare, Square } from "@lucide/vue";
...
const messagePreviewLocale = computed(() => localeConfig.value.defaultLocale || "zh-CN");
</script>
...
<template>
  <div
    ...
    :style="{ 'padding-left': `calc(var(--spacing) * 3 * ${item.depth} + 6px)` }"
    :class="
      cn(
        ...
        {
          'border-l-blue-600 rounded-l-none bg-zinc-100': isSelected(item.id) || isChecked(item.id),
        },
        props?.class,
      )
    "
  ...
  >
    ...
    >
      <CheckSquare v-if="isChecked(item.id)" class="size-4 text-primary" />
      <MinusSquare v-else-if="isIndeterminate(item.id)" class="size-4 text-primary" />
      <Square v-else class="size-4" />
    </button>
    <label
      :class="{ 'text-muted-foreground': !item.original.translations[messagePreviewLocale] }"
    ...
    <label
      v-else
      :class="{ 'text-muted-foreground': !item.original?.title && !item.original?.key }"
    >
      <template v-if="!item.original?.title && !item.original?.key"> 未命名 </template>
      <template v-else-if="!item.original?.title">
        {{ item.original.key }}
      </template>
      <template v-else>
        <span>{{ item.original.title }}</span>
        <small v-if="!item.original?.key" class="text-destructive ml-1"> &lt;无 Key&gt; </small>
        <small v-else class="text-muted-foreground ml-1 font-mono">
          {{ item.original.key }}
        </small>
      </template>
    </label>
    <WorkspaceRowActions
      ...
      :item="item"
    />
    <FolderTree class="text-slate-400 size-3.5 mt-1" v-if="item.original.type === 'group'" />
    <Languages class="text-slate-400 size-3.5 mt-1" v-else />
  </div>
</template>
