<template>
  <footer class="flex items-center justify-between gap-4 p-1 px-2 text-xs">
    <p v-if="document.isLoading" class="text-muted-foreground">正在加载 /api/data ...</p>
    <span v-else-if="document.error && !document.hasLoaded" class="text-destructive">
      加载失败：{{ document.error }}
    </span>
    ...
    <div class="flex items-center gap-3">
      <Status :variant="syncStatusVariant" class="text-xs">
        <small>{{ syncStatusText }}</small>
      </Status>
      <Status :variant="statusVariant" class="text-xs">
...
import { type StatusVariants, Status } from "@/components/ui/status";
