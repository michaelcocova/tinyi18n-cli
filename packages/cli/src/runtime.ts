...
import { createShortcutHelp, filterErrorLog, formatStartupInfo, printLines } from "./output.ts";
...
export function resolveRuntimeMode(env: NodeJS.ProcessEnv = process.env): TinyI18nMode {
...
export function startWebDevServer(
  ...
  const child = spawn(
    "vp",
    ["dev", "--", "--host", hostname, "--port", String(port), "--strictPort"],
    {
      cwd: webPackageRoot,
      env: {
        ...process.env,
