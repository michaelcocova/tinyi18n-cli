...
export function formatStartupInfo(info: StartupInfo) {
  ...
  if (info.urls.network.length > 0) {
    info.urls.network.forEach((url) => {
      lines.push(`  ${green("➜")}  ${bold("网络:")}   ${cyan(withTrailingSlash(url))}`);
    });
  } else {
    lines.push(`  ${green("➜")}  ${bold("网络:")}   ${dim("使用 --host 暴露服务到局域网")}`);
  }
...
export function filterErrorLog(chunk: string) {
  ...
  const firstErrorIndex = lines.findIndex((line) => line.toLowerCase().includes("error"));
