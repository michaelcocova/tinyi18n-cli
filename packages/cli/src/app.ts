...
import { formatStartupInfo, printIgnoredDevelopmentPortNotice, printLines } from "./output.ts";
...
export async function runCli(args: string[] = process.argv.slice(2), cwd: string = process.cwd()) {
  ...
  if (command.type === "sync") {
    ...
    if (!result.ok) {
      console.error(red(`同步失败：${result.error}`));
      process.exitCode = 1;
    } else {
      console.log(green(`同步成功：共生成 ${result.files.length} 个文件`));
      process.exitCode = 0;
    }
    return;
  }
