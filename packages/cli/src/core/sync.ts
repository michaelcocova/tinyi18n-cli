...
import type { TinyI18nItem, TinyI18nMessage, TinyI18nSnapshot } from "./message.ts";
...
function resolveMessagePath(message: TinyI18nMessage, itemsById: Map<string, TinyI18nItem>) {
  ...
  while (parentId) {
    ...
    if (parent.type === "message") {
      throw new Error(`Message "${message.key}" cannot be nested under another message`);
    }
  ...
  for (const [index, segment] of segments.entries()) {
    ...
    if (isLeaf) {
      if (current != null && typeof current === "object") {
        throw new Error(`Cannot overwrite object path "${fullPath}" in locale "${locale}"`);
      }
...
function createSyncPlan(snapshot: TinyI18nSnapshot) {
  ...
  syncableMessages.sort((left, right) => left.path.join(".").localeCompare(right.path.join(".")));
...
function createLocalePayload(
  ...
  for (const entry of syncableMessages) {
    setNestedValue(payload, entry.path, entry.message.translations[locale] ?? "", locale);
  }
  ...
  return (config.entries ?? []).map((entry) => {
    ...
    const entryMessages =
      !entry.paths || entry.paths.length === 0
        ? syncableMessages
        : syncableMessages.filter((item) => new Set(entry.paths).has(item.path[0]));
...
export async function syncWorkspaceToProjectFiles(
  ...
  try {
    ...
    if (!snapshot.initialized) {
      return { ok: false, error: "工作区未初始化，请先运行 `tinyi18n ui` 命令进行配置" };
    }
    ...
    const entrySyncPlan = createEntrySyncPlan(snapshot.config, syncableMessages);
